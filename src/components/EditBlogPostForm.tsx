...
export default function EditBlogPostForm({ post }: { post: PlainPostType }) {
  ...
  return (
    <form action={formAction} className="space-y-6 bg-white p-8 rounded-lg shadow-md">
      {/* This section displays success or error messages from the server */}
      {state?.error && <p className="text-red-500 text-sm">{state.error}</p>}
      {state?.message && <p className="text-green-600 text-sm">{state.message}</p>}

      {/* The post._id is now a guaranteed string, so no more errors here */}
      ...
      <div>
        <label htmlFor="image" className="block text-sm font-medium text-gray-700">Change Featured Image (Optional)</label>
        <div className="mt-2 flex items-center gap-4">
          ...
          <input type="file" name="image" id="image" accept="image/*" className="block w-full text-sm text-gray-500 file:mr-4 file:py-2 file:px-4 file:rounded-md file:border-0 file:text-sm file:font-semibold file:bg-red-50 file:text-red-700 hover:file:bg-red-100" />
        </div>
      </div>
